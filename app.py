...
def login():
    if 'user_id' in session:
        ...
        if user:
            ...
            if user.department:
                ...
                else: flash(f"Your assigned department is inactive.", "danger"); return redirect(url_for('logout'))
    ...
    if request.method == 'POST':
        ...
        if user and user.password == request.form.get('password'):
            ...
            if user.role == 'superadmin': return redirect(url_for('admin_panel'))
            elif user.department and user.department.active: return redirect(url_for('department_dashboard', department_name=user.department.name))
            else: flash(f"Your assigned department is inactive.", "danger"); return redirect(url_for('logout'))
        else:
...
def setup_page():
    ...
    if request.method == 'POST':
        ...
        if user.role == 'superadmin':
            flash("Superadmin username cannot be changed from this page for security reasons.", 'warning')
        elif new_username:
             if User.query.filter(User.username == new_username, User.id != user.id).first():
                 flash("Username already taken.", 'danger')
             else:
                 user.username = new_username; db.session.commit()
                 flash("Username changed successfully! Please log in again.", 'success'); return redirect(url_for('logout'))
...
def add_user():
    user, is_super_admin = get_current_user_and_permissions();
    if not is_super_admin: return redirect(url_for('login'))
    username, email, password, role, department_id = (request.form.get(k) for k in ['username','email','password','role','department_id'])
    if User.query.filter(or_(User.username == username, User.email == email)).first():
        flash('Username or email already exists.', 'danger')
    else:
        new_user = User(username=username, email=email, password=password, role=role) # In prod, hash the password
...
def edit_user(user_id):
    ...
    if new_password: user_to_edit.password = new_password # In prod, use set_password() to hash it
...
def search():
    ...
    posts_found = Post.query.join(Menu).join(Department).filter(q_filter, not_(Post.is_deleted), or_(Post.rich_content.ilike(search_term), Post.scraped_file_content.ilike(search_term))).all()
    
    for post in posts_found:
        # Simplified context for now
        results.append({'type': 'Content', 'url': url_for('menu_content_page', department_name=post.menu.department.name, menu_id=post.menu.id), 'department': post.menu.department.name, 'menu': post.menu.name, 'context': f"Found in content of {post.menu.name}"})
...
def toggle_post_status(post_id):
    ...
    if post and can_user_edit_department(user, post.menu.department, is_super_admin):
        post.active = not post.active; db.session.commit(); flash("Post status updated.", 'info')
    return redirect(request.referrer)
...
def download_file(file_id):
    ...
    can_edit = can_user_edit_department(user, attachment.post.menu.department, is_super_admin)
    if (not attachment.post.menu.active or not attachment.post.active) and not can_edit:
