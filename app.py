...
def get_flat_menu_list(menus):
    ...
    def _flatten(menu_list, depth=0):
        for item in menu_list:
            ...
            if item.children:
                _flatten(item.children, depth + 1)
    _flatten(menus)
    return flat_list
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and user.password == request.form.get('password'):
            # SUCCESS: Set session and redirect to the CORRECT destination.
            ...
            # FAILURE: Flash error and re-render the login page. No redirect.
            flash("Invalid username or password.", "danger")
            return render_template('login.html')
    
    # If it's a simple GET request, just show the login page.
    return render_template('login.html')
...
def department_dashboard(department_name):
    ...
    if request.method == 'POST' and can_edit:
        new_menu_name = request.form.get('new_menu_name')
        if new_menu_name:
            parent_id = request.form.get('parent_menu_id'); new_menu = Menu(name=new_menu_name, department_id=department.id)
    ...
    return render_template('department_dashboard.html', department=department,
                           top_level_menus=top_level_menus, flat_menu_list=get_flat_menu_list(top_level_menus),
    ...
    if request.method == 'POST' and can_edit:
        ...
        for file in files:
            if file and file.filename:
                filename = secure_filename(file.filename); upload_path = os.path.join(app.config['UPLOAD_FOLDER'], str(menu.department_id), str(menu.id))
...
def search():
    ...
    q_filter = or_(Department.active == True, is_super_admin == True)
    posts_found = Post.query.join(Menu).join(Department).filter(q_filter, or_(Post.rich_content.ilike(search_term), Post.scraped_file_content.ilike(search_term))).all()
    files_found = FileAttachment.query.join(Post).join(Menu).join(Department).filter(q_filter, FileAttachment.filename.ilike(search_term)).all()

    for post in posts_found:
        results.append({'type': 'Content', 'department': post.menu.department.name, 'menu': post.menu.name, 'url': url_for('menu_content_page', department_name=post.menu.department.name, menu_id=post.menu.id)})
    for attachment in files_found:
        results.append({'type': 'File', 'department': attachment.post.menu.department.name, 'menu': attachment.post.menu.name, 'url': url_for('download_file', file_id=attachment.id), 'context': f"{attachment.filename}"})
    return render_template('search_result.html', query=request.args.get('query',''), results=results, user=user)
...
def toggle_post_status(post_id):
    ...
    if post and can_user_edit_department(user, post.menu.department, is_super_admin):
        post.active = not post.active; db.session.commit()
    return redirect(request.referrer)
...
def download_file(file_id):
    ...
    if not attachment.post.active and not can_edit: return "Access to this file's post is restricted.", 403
